...
def control_thread():
    ...
    while system_running:
        ...
        if mode_switch.is_drawing:
            # 1. 时间开环计算相位 (底盘 16秒 一圈)
            lap_time = 16.0 
            elapsed = time.time() - mode_switch.start_time
            
            # 算出当前时间占比，并转为 0~2pi 的弧度相位
            phase = 2 * math.pi * (elapsed / lap_time)
            
            # 若发现画圈方向与小车行进方向反了，取消下行注释反转相位：
            # phase = -phase

            # 2. 斜视投影畸变补偿 (完全保留该神级优化)
